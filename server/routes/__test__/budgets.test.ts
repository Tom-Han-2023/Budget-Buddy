...
describe('GET /api/v1/budgets', () => {
  ...
  it('should return status code 500 if there is error getting the budget from database', async () => {
...
describe('POST /api/v1/budgets', () => {
  ...
  it('should return the back the newly added budget back  if authorised correctly', async () => {
    jest
      .mocked(checkJwt)
      .mockImplementation(async (req: JwtRequest, res, next) => {
        req.auth = {
  ...
  it('should return status code 500 if there is error adding the budget from database', async () => {
    jest.spyOn(console, 'log').mockImplementation(() => {})
    jest
      .mocked(checkJwt)
      .mockImplementation(async (req: JwtRequest, res, next) => {
...
describe('PATCH /api/v1/budgets/:id', () => {
  ...
  it('should update the budget and return details up the updated budget', async () => {
    jest
      .mocked(checkJwt)
      .mockImplementation(async (req: JwtRequest, res, next) => {
        req.auth = {
  ...
  it('should return status code 500 if there is error updating the budget from database', async () => {
    // jest.spyOn(console, 'log').mockImplementation(() => {})
    jest
      .mocked(checkJwt)
      .mockImplementation(async (req: JwtRequest, res, next) => {
